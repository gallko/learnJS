...
let player;
const videosWrepper = document.querySelector('.videos__wrapper');
...
//             </div>
//         `;
//         videosWrepper.appendChild(card);
//         setTimeout(() => {
...
function start() {
    gapi.client.init({
        'apiKey': 'AIzaSyDHDTT_-dOAGdwyJm2oA84Nf8r3duit9QI',
...
function search(target) {
    gapi.client.init({
        ...
        console.log(response.result);
        // videosWrepper.innerHTML = '';
        while (videosWrepper.firstChild) {
            videosWrepper.removeChild(videosWrepper.firstChild);
        }
...
function bindModal(cards) {
    cards.forEach(item => {
        item.addEventListener('click', (e) => {
            e.preventDefault(); // отмена ссылок
